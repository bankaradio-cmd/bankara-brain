#!/usr/bin/env python3
"""バンカラブレイン — 全60本をGemini 3.1 Proで再分析（新プロンプト版）

使い方:
    cd バンカラブレイン
    python3 scripts/batch_gemini_full_reanalysis.py
"""
...
MAX_RETRIES = 4
BASE_DELAY = 15.0
WAIT_BETWEEN = 30  # 動画間の待機秒数

# ============================================================
# 改善版プロンプト（エネルギー・感情トーン・強み弱み削除済み）
# ============================================================
...
def backup_database(db_path: Path) -> Path:
    ...
    backup_path = db_path.parent / f"{db_path.name}.backup_gemini_reanalysis_{timestamp}"
    if backup_path.exists():
        print(f"  バックアップ済み: {backup_path.name}")
    else:
        shutil.copy2(db_path, backup_path)
        size_mb = backup_path.stat().st_size / (1024 * 1024)
        print(f"  バックアップ作成: {backup_path.name} ({size_mb:.1f}MB)")
    return backup_path


def get_all_videos(db_path: Path) -> list[dict]:
    """全動画のリストを返す（gemini_analysisの有無に関係なく）"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"バンカラブレイン — Gemini 全動画再分析（新プロンプト版）")
    print(f"{'='*60}")
    print(f"対象動画: {total}本（全動画）")
    print(f"モデル: {MODEL_NAME}")
    print(f"方式: YouTube URL直接渡し（映像分析）")
    print(f"推定所要時間: 約{estimated_minutes:.0f}分（{estimated_hours:.1f}時間）")
    print(f"  ※ 1本あたり約1.5分（分析 + {WAIT_BETWEEN}秒待機）")
    print(f"{'='*60}")
    ...
    for i, video in enumerate(videos, 1):
        print(f"  [{i:2d}/{total}] {video['title']} ({video['duration_seconds']/60:.1f}分)")
    ...
    for i, video in enumerate(videos, 1):
        ...
        if i > 1:
            ...
            remaining = avg_per_video * (total - i + 1)
            remaining_min = remaining / 60
            eta_str = f" | 残り約{remaining_min:.0f}分"
        else:
            eta_str = ""

        print(f"[{i}/{total}] 分析中: {title}{eta_str}")
